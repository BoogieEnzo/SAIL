...
let abi_mips64_data_encoding:Nat_big_num.num=  elf_data_2lsb

(*val abi_mips64_endianness : endianness*)
let abi_mips64_endianness:endianness=  Little (* Must match above *)
   ...
   ((match Lem_list.list_index magic (Nat_big_num.to_int elf_ii_class) with
    ...
    | Some cls ->
      (match Lem_list.list_index magic (Nat_big_num.to_int elf_ii_data) with
        | None   -> false
        | Some data ->
            ( Nat_big_num.equal(Uint32_wrapper.to_bigint cls) abi_mips64_file_class) &&
              ( Nat_big_num.equal(Uint32_wrapper.to_bigint data) abi_mips64_data_encoding)
      )
  ))
