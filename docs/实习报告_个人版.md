...
- **为什么分这么多目录**：ACT 测试依赖“参考模型 + 模拟器 + 工具链 + 脚本”。参考模型在 `sail-riscv`，模拟器 Spike 装在 `tools/spike`，工具链在 `tools` 下，自动化流程在 `scripts`，真正要改的测试代码在子仓库 `riscv-arch-test` 里；其它目录有的是历史/演示（可删），有的是文档和证据（需保留）。下面「当前文件结构」会标清哪些必须保留、哪些可以不要。
- **开发流程大概几步**：  
  ...
  3）在 GitHub 上 fork 的仓库里配好 CI（regression）和 OpenCode（`/oc` 触发），PR 时以 CI 结果为准。  
...
## 当前文件结构（便于汇报与判断哪些目录可删）

根目录一级结构（`tree -L 1`，按当前仓库实际）：

```
.
├── docs                    # 实习报告、心跳、说明文档，需保留（运行日志在 docs/automation-logs）
├── riscv-arch-test         # ACT 仓库（Zabha 测试与 CI 均在此），需保留
├── sail-riscv              # 参考模型 sail_riscv_sim 的源码/构建，需保留
├── scripts                 # 阶段化执行与诊断脚本，需保留
└── tools                   # spike、工具链等构建产物与安装目录，需保留
```

- **必须保留**：`docs`、`riscv-arch-test`、`sail-riscv`、`scripts`、`tools`（含 `tools/spike`）。
- **可保留**：`docs/automation-logs`（运行日志，便于排查与汇报）。
- **可不要**：若存在 `riscv-isa-sim`（Spike 源码），可删，脚本只依赖 `tools/spike`。

汇报时可直接引用上述结构；主线依赖五类目录，运行日志在 `docs/automation-logs`，其余无需重构。
